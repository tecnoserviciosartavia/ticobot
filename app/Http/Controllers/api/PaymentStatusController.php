...
class PaymentStatusController extends Controller
{
    ...
    public function clientsWithPendingPayments(Request $request)
    {
        ...
        // Regla de negocio: "deuda" = hay cobro esperado (contrato) y NO existe pago registrado.
        // Para evitar falsos positivos/negativos, detectamos contratos cuyo next_due_date ya pasó
        // (o está por vencer) y verificamos que no exista un Payment en el mismo mes del due.
        // Nota: esto NO depende de conciliación ni de status "unverified".
        $today = Carbon::today();
        $query = Client::query()
            ...
            ->with(['contracts' => function ($q) {
                $q->whereNotNull('next_due_date')->orderBy('next_due_date', 'asc');
            }]);
        ...
        $data = $clients->map(function ($client) {
            ...
            foreach ($client->contracts as $contract) {
                ...
                $due = Carbon::parse($contract->next_due_date);

                // Consideramos pendiente si el contrato ya está vencido o vence hoy.
                // (Podemos extenderlo a "vence pronto" pero la vista es de pendientes.)
                if ($due->isAfter(Carbon::today())) {
                    continue;
                }

                $hasPaymentForDueMonth = Payment::query()
                    ->where('contract_id', $contract->id)
                    ->whereYear('created_at', $due->year)
                    ->whereMonth('created_at', $due->month)
                    ->exists();
                ...
                $pendingCount++;
                $pendingAmount += (float) $contract->amount;
        ...
        })->filter(fn ($row) => ($row['pending_payments_count'] ?? 0) > 0)->values();
    ...
    {
        // Resumen de pendientes basado en contratos vencidos SIN pago registrado en el mes del due.
        // Hacemos la agregación en PHP para mantenerlo correcto y legible; si crece mucho, se optimiza
        // con una query agregada.
        ...
        foreach ($clients as $client) {
            ...
            foreach ($client->contracts as $contract) {
                ...
                $clientPending++;
                $totalPendingCount++;
                $totalPendingAmount += (float) $contract->amount;
                $currency = $contract->currency ?? 'CRC';
                if (!isset($byCurrency[$currency])) {
                    $byCurrency[$currency] = ['currency' => $currency, 'total' => 0, 'count' => 0];
                }
                $byCurrency[$currency]['total'] += (float) $contract->amount;
        ...
        $byStatus = array_values($byCurrency);
