...
export default function FinanceNav({ active, periodLabel }: { active: FinanceSection; periodLabel?: string }) {
    ...
    return (
        <div className="space-y-5">
            <div className="rounded-2xl bg-gradient-to-r from-emerald-700 to-teal-600 p-5 text-white shadow-sm sm:p-7">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                    <div>
                        ...
                        <p className="mt-2 max-w-2xl text-sm text-emerald-50">Registrá, revisá y administrá las finanzas desde un solo lugar.</p>
                    </div>
                    <Link href={route('payments.create')} className="inline-flex h-11 items-center justify-center rounded-xl bg-white px-4 text-sm font-semibold text-emerald-800 shadow-sm">
            ...
            <nav className="flex gap-2 overflow-x-auto rounded-2xl border border-gray-200 bg-white p-2 dark:border-gray-700 dark:bg-gray-800">
                {items.map(({ id, label, href, icon: Icon }) => (
                    <Link key={id} href={href} preserveState className={`inline-flex min-w-max items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-semibold ${active === id ? 'bg-emerald-600 text-white' : 'text-gray-600 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-700'}`}>
