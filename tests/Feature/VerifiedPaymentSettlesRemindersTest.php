...
class VerifiedPaymentSettlesRemindersTest extends TestCase
{
    ...
    public function test_manual_verified_payment_marks_duplicate_pending_reminders_for_same_due_datetime_as_paid(): void
    {
        config()->set('app.timezone', 'America/Costa_Rica');
        config()->set('reminders.send_time', '09:00');
        ...
        $originalReminder = Reminder::query()->where('contract_id', $contract->id)->firstOrFail();
        $duplicateReminder = Reminder::create([
            'client_id' => $client->id,
            'contract_id' => $contract->id,
            'channel' => 'whatsapp',
            'scheduled_for' => $originalReminder->scheduled_for,
            'status' => 'pending',
        ]);
        ...
        $originalReminder->refresh();
        $duplicateReminder->refresh();

        $this->assertSame('paid', $originalReminder->status);
        $this->assertSame('paid', $duplicateReminder->status);
        $this->assertNotNull($originalReminder->acknowledged_at);
        $this->assertNotNull($duplicateReminder->acknowledged_at);
    }
}
